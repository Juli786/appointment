class Calendar {
    ...
    update() {
        ...
        getAvailableSolt((this.selected._i) ? this.selected._i : moment().format('YYYY-MM-DD'))
        this.calendarDays = {
    ...
    draw() {
        ...
        for (let day = 1; index <= this.bodyDivs.length - 1; index++) {
            ...
            $(".display-selected").text(`${getDayName(this.selected._d)} ${this.selected.date()}`);
...
function getDayName(date = new Date(), locale = 'en-US') {
    return `${ date.toLocaleDateString(locale, { weekday: 'long' })},${ date.toLocaleDateString(locale, { month: 'long' }) }`;
